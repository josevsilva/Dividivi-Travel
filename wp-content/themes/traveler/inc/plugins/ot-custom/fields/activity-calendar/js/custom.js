			...
			self.calendar.fullCalendar({
				...
				select : function(start, end, jsEvent, view){
					...
					if((start_date < today && start_year <= today_year) || (end_date < today && end_year <= today_year)){
						...
						var check_in = moment(start._d).utcOffset(zone).format( String(st_params.dateformat || "MM/DD/YYYY").toUpperCase() );
						var	check_out = moment(end._d).utcOffset(zone).subtract(1, 'day').format( String(st_params.dateformat || "MM/DD/YYYY").toUpperCase());
				...
				eventClick: function(event, element, view){
                    setCheckInOut(event.start.format( String(st_params.dateformat || 'MM/DD/YYYY').toUpperCase()), event.start.format(String(st_params.dateformat || 'MM/DD/YYYY').toUpperCase()), self.form_container);
